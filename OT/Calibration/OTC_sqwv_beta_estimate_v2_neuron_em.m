...
% datdir='/Users/emilyprowse/Documents/McGill/Hendricks_Lab/Data/Pamily_OT/Neurons!/20220213_30Q_DIV7_OT/';
% datdir = '/Volumes/Emily_htt_2/Neuron/isoHD18Q/20230209_18Q_lyso_bdnf_OT/OT/'; % defining the path to where the transferfunction.mat files are
% datdir = '/Volumes/Emily_htt_2/Neuron/isoHD81Q/20230603_81Q_OT/';
...
beta_datdir = '/Volumes/Emily_2022/Neurons/20240416_kolf4_OT/';
% 
% %set filename to multiharmonic measurement
beta_fname = '20240416_kolf4_OT_s3_beta_6.txt';
...
load_file_ig = 1; % load the initial guess conditions from a previous iteration that you will have saved
user_ig = 0; % this uses a fully hardcoded initial guess from the user and, similar to the load_file_ig, completely skips the interactive steps for finding a proper initial guess 
...
alphaLB = 0.3; %lower bound on the alpha value (-Ora) 
alphaUB = 1.7; %upper bound on the alpha value (-Ora)

ktrap_lb = 0.0001;
%ktrap_ub = 0.1;

%set initial guess
% beta_guess = 80000; %Emily arbitrarily set this value as the initial guess, modify based on fit and whether it's hitting the bounds
...
ktrap_guess = 0.02; %0.05; %0.1; % manual input, Emily changed from 0.1 to 0.05
...
betalb = 0.9; %lower bound on beta Emily changed to 0.5 from 0.9
...
% Wmag = 5e-3; %Emily changed from 5e-2. 0.5e-3; weight applied to the magnitude. This is key in balancing the quality of the fit between the FRF and the PS. A low weight here will fit the power spectrum better, but the TF worse. In general, it's better to prioritize the power spectrum as it is more reliable because of the number of points.
...
elseif numPS == 2
    ...
    usePS = 2; %input('Which PS do you want to use for calibration? (1 - PSx, 2 - PSy) '); %Emily-use PSy, it has less noise from excitation
        if usePS == 1
            FP = FP_x;
            PYm = PYm_x;
            PSfit = '-xPS';
...
%Emily modifying to include more of the "hump" (23/06/2022)
Fexc_PS_reject=[Fexcall,55:57,70:71,75:95,135:180,235:250,650:730,260:280,905:915,330:395,660:720,760:800,845,686,382,923.5,6172,7445,7490,8000,8800,2950,810:830];%600:760];%1700:2200,2920,2815,5250,6500:6580,7600:7650];%,random_noise,random_noise2,random_noise4,random_noise9,2815,3853,random_noise10,random_noise11];%random_noise2,random_noise8,random_noise5];
%keeping more of the "hump" for y PS (Ora)
...
% freq_low=13; %Emily modified to get rid of the clicking and make analysis faster
delete(t1)
...
% freq_high=10000; %Emily modified to get rid of the clicking and make the code run faster
delete(t1)
...
if semi_auto_ig == 1 % in that case, the user has to click values multiple times
    ...
    set(gca,'Ylim',[1e-14 1]); %Emily changed from 1e-12 to 1e-14
    ...
    [fmin,ymin] = ginput(1); 
%     [fmin,ymin] = [50,1e-10]; 
%     fmin=50; %Emily changed to fit the same frequency range for all
%     calibrations, but optimized for HEKs
    [fmax,ymax] = ginput(1);
%     [fmax,ymax] = [2500,3e-14];
%     fmax=1500; %Emily changed, but optimized for HEKs
    delete(t1)
    freqplot = logspace(-2,4,350); % this makes 350 log-evenly spaced points, from 10^-2 to 10^4, and is used for plotting the functions. 
end
...
falias=20000;%6e3;%20e3; %falias:  QPD has filtering effect. first order
% low pass filter , Emily Modified May 24 2022 to 50000, Aug 23 2022 to
% 30000
% falias=1000;%6e3;%20e3; %falias:  QPD has filtering effect. first order low pass filter
...
%Emily changing stuff 20240426
    caldir='/Volumes/Emily_2022/Neurons/Good_OT_Calibrations/';
    cd(caldir)
    u2_OLD=load('18.04.24-08.53-TF-20240416_kolf4_OT_s1_cal_5-OTC-betasqwv-fixnfit-r10-fitOrder0-fitGroups1-hardcoded-guess-yPS-fmin14.0163.mat');
...
%     ig.k_cyt1 = u0_loaded(6);
    ...
    ig.Beta = u0_loaded.beta;
    ig.GammaR = u0_loaded.gammar;
    ig.Alpha = u0_loaded.alpha;
    ig.k_trap = u0_loaded.k_trap;
    ig.k_cyt0 = u0_loaded.k_cyt0;
    ig.k_cyt1 = u0_loaded.k_cyt1;
...
% lb=[beta_lb     0.1*ig.GammaR    0.95*ig.Alpha     0.25.*ig.k_trap   0.1*ig.k_cyt0    0.1*ig.k_cyt1    1e-1    0]; %Emily modified Jun 11/22
% ub=[beta_ub     10*ig.GammaR     1.05*ig.Alpha     20*ig.k_trap     10.*ig.k_cyt0    10*ig.k_cyt1    1e1     1.01]; %Emily modified Jun 11/22
