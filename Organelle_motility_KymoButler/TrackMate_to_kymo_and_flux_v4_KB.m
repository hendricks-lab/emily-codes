...
%flux analysis: how many trajectories cross a certain sized box in the axon?
% position=r0{kd}; %without kymobutler
    t_position=kbpos(kd).tk;% Time (s)
    position=kbpos(kd).position; %with kymobutler
for it=1:5
    box_width=7.5; %We want to look +/- box_width from the middle of the axon
    ax_seg=(axon_length{k}*it)/5-(box_width+1); %segment of the axon
...
save([save_dir, 'KB_total_enters_IFg_15um'],'tot_enters_per_cell_all_conds');
save([save_dir, 'KB_frac_ant_ent_IFg_15um'],'frac_ant_enter_per_cell_all_conds');
save([save_dir, 'KB_frac_ret_ent_IFg_15um'],'frac_ret_enter_per_cell_all_conds');
save([save_dir, 'KB_start_in_IFg_15um'],'frac_starts_in_per_cell_all_conds');
save([save_dir, 'KB_frac_ant_ex_IFg_15um'],'frac_ant_exits_per_cell_all_conds');
save([save_dir, 'KB_frac_ret_ex_IFg_15um'],'frac_ret_exits_per_cell_all_conds');
save([save_dir, 'KB_frac_stay_in_IFg_15um'],'frac_stays_in_per_cell_all_conds');
...
xticklabels({'30Q','30Q IFNγ', '81Q', '81Q IFNγ'});
ylabel('Ant/Ret Flux Entry');
publication_fig(0,0,1);
box("off");
...
xticklabels({'30Q','45Q', '65Q', '81Q'});
% xticklabels({'30Q','30Q IFNγ', '81Q', '81Q IFNγ'});
ylabel('Ant/Ret Flux Exit');
publication_fig(0,0,1);
box("off");

figure('Name','total_entered','NumberTitle','off'), hold on, %4 conditions
scatter(1-0.125+0.25*rand(numel(total_enters_per_cell_value_all_conds{1}),1),total_enters_per_cell_value_all_conds{1}./(box_width*2*5),25,'MarkerFaceColor','none','MarkerEdgeAlpha',0.3,'MarkerEdgeColor',colour_30Q,'LineWidth',2), hold on;
scatter(2-0.125+0.25*rand(numel(total_enters_per_cell_value_all_conds{2}),1),total_enters_per_cell_value_all_conds{2}./(box_width*2*5),25,'MarkerFaceColor','none','MarkerEdgeAlpha',0.3,'MarkerEdgeColor',colour_45Q,'LineWidth',2);
scatter(3-0.125+0.25*rand(numel(total_enters_per_cell_value_all_conds{3}),1),total_enters_per_cell_value_all_conds{3}./(box_width*2*5),25,'MarkerFaceColor','none','MarkerEdgeAlpha',0.3,'MarkerEdgeColor',colour_65Q,'LineWidth',2);
scatter(4-0.125+0.25*rand(numel(total_enters_per_cell_value_all_conds{4}),1),total_enters_per_cell_value_all_conds{4}./(box_width*2*5),25,'MarkerFaceColor','none','MarkerEdgeAlpha',0.3,'MarkerEdgeColor',colour_81Q,'LineWidth',2);
plot(1,mean(total_enters_per_cell_value_all_conds{1}./(box_width*2*5),"omitnan"),'k_','MarkerSize',30);
plot(2,mean(total_enters_per_cell_value_all_conds{2}./(box_width*2*5),"omitnan"),'k_','MarkerSize',30);
plot(3,mean(total_enters_per_cell_value_all_conds{3}./(box_width*2*5),"omitnan"),'k_','MarkerSize',30);
plot(4,mean(total_enters_per_cell_value_all_conds{4}./(box_width*2*5),"omitnan"),'k_','MarkerSize',30);
xlim([0 5]);
xticks([1 2 3 4]);
publication_fig(0,0,1);
...
ant_vs_ret_all{1}=ant_vs_ret_raw{1}(isfinite(ant_vs_ret_raw{1}));
ant_vs_ret_all{2}=ant_vs_ret_raw{2}(isfinite(ant_vs_ret_raw{2}));
ant_vs_ret_all{3}=ant_vs_ret_raw{3}(isfinite(ant_vs_ret_raw{3}));
ant_vs_ret_all{4}=ant_vs_ret_raw{4}(isfinite(ant_vs_ret_raw{4}));
...
SEM_tot_ent{1}=std(tot_enters_per_cell_all_conds{1}./(box_width*2*5))/sqrt(length(tot_enters_per_cell_all_conds{1}));
SEM_tot_ent{2}=std(tot_enters_per_cell_all_conds{2}./(box_width*2*5))/sqrt(length(tot_enters_per_cell_all_conds{2}));
SEM_tot_ent{3}=std(tot_enters_per_cell_all_conds{3}./(box_width*2*5))/sqrt(length(tot_enters_per_cell_all_conds{3}));
SEM_tot_ent{4}=std(tot_enters_per_cell_all_conds{4}./(box_width*2*5))/sqrt(length(tot_enters_per_cell_all_conds{4}));

SEM_ant_ret{1}=std(ant_vs_ret_all{1})/sqrt(length(ant_vs_ret_all{1}));
SEM_ant_ret{2}=std(ant_vs_ret_all{2})/sqrt(length(ant_vs_ret_all{2}));
SEM_ant_ret{3}=std(ant_vs_ret_all{3})/sqrt(length(ant_vs_ret_all{3}));
SEM_ant_ret{4}=std(ant_vs_ret_all{4})/sqrt(length(ant_vs_ret_all{4}));
...
ts_ant_ret{1} = tinv([0.025  0.975],length(ant_vs_ret_all{1})-1);      % T-Score
ts_ant_ret{2} = tinv([0.025  0.975],length(ant_vs_ret_all{2})-1); 
ts_ant_ret{3} = tinv([0.025  0.975],length(ant_vs_ret_all{3})-1); 
ts_ant_ret{4} = tinv([0.025  0.975],length(ant_vs_ret_all{4})-1); 
...
CI_ant_ret{1} = mean(ant_vs_ret_all{1}) + ts_ant_ret{1}*SEM_ant_ret{1};   
CI_ant_ret{2} = mean(ant_vs_ret_all{2}) + ts_ant_ret{2}*SEM_ant_ret{2};
CI_ant_ret{3} = mean(ant_vs_ret_all{3}) + ts_ant_ret{3}*SEM_ant_ret{3}; 
CI_ant_ret{4} = mean(ant_vs_ret_all{4}) + ts_ant_ret{4}*SEM_ant_ret{4}; 

figure('Name','total_enters_inset','NumberTitle','off'), hold on,
scatter(1-0.125+0.25*rand(numel(tot_enters_per_cell_all_conds{1}),1),tot_enters_per_cell_all_conds{1}./(box_width*2*5),25,'MarkerFaceColor','none','MarkerEdgeColor',colour_30Q,'MarkerEdgeAlpha',0.5,'LineWidth',2);
scatter(2-0.125+0.25*rand(numel(tot_enters_per_cell_all_conds{2}),1),tot_enters_per_cell_all_conds{2}./(box_width*2*5),25,'MarkerFaceColor','none','MarkerEdgeColor',colour_45Q,'MarkerEdgeAlpha',0.5,'LineWidth',2);
scatter(3-0.125+0.25*rand(numel(tot_enters_per_cell_all_conds{3}),1),tot_enters_per_cell_all_conds{3}./(box_width*2*5),25,'MarkerFaceColor','none','MarkerEdgeColor',colour_65Q,'MarkerEdgeAlpha',0.5,'LineWidth',2);
scatter(4-0.125+0.25*rand(numel(tot_enters_per_cell_all_conds{4}),1),tot_enters_per_cell_all_conds{4}./(box_width*2*5),25,'MarkerFaceColor','none','MarkerEdgeColor',colour_81Q,'MarkerEdgeAlpha',0.5,'LineWidth',2);
plot(1,mean(tot_enters_per_cell_all_conds{1}./(box_width*2*5)),'k_','MarkerSize',10);
plot(2,mean(tot_enters_per_cell_all_conds{2}./(box_width*2*5)),'k_','MarkerSize',10);
plot(3,mean(tot_enters_per_cell_all_conds{3}./(box_width*2*5)),'k_','MarkerSize',10);
plot(4,mean(tot_enters_per_cell_all_conds{4}./(box_width*2*5)),'k_','MarkerSize',10);
errorbar(1,mean(tot_enters_per_cell_all_conds{1}./(box_width*2*5)),SEM_tot_ent{1},'Color',colour_30Q,'LineWidth',2,'CapSize',0);
errorbar(2,mean(tot_enters_per_cell_all_conds{2}./(box_width*2*5)),SEM_tot_ent{2},'Color',colour_45Q,'LineWidth',2,'CapSize',0);
errorbar(3,mean(tot_enters_per_cell_all_conds{3}./(box_width*2*5)),SEM_tot_ent{3},'Color',colour_65Q,'LineWidth',2,'CapSize',0);
errorbar(4,mean(tot_enters_per_cell_all_conds{4}./(box_width*2*5)),SEM_tot_ent{4},'Color',colour_81Q,'LineWidth',2,'CapSize',0);
xlim([0 5]);
xticks([1 2 3 4]);
% xticklabels({'30Q','45Q', '65Q','81Q'});
xticklabels({'30Q','30Q+IFNγ', '81Q','81Q+IFNγ'});
...
figure('Name','ant_vs_ret_inset','NumberTitle','off'), hold on,
scatter(1-0.125+0.25*rand(numel(ant_vs_ret_all{1}),1),ant_vs_ret_all{1},25,'MarkerFaceColor','none','MarkerEdgeColor',colour_30Q,'MarkerEdgeAlpha',0.5,'LineWidth',2);
scatter(2-0.125+0.25*rand(numel(ant_vs_ret_all{2}),1),ant_vs_ret_all{2},25,'MarkerFaceColor','none','MarkerEdgeColor',colour_45Q,'MarkerEdgeAlpha',0.5,'LineWidth',2);
scatter(3-0.125+0.25*rand(numel(ant_vs_ret_all{3}),1),ant_vs_ret_all{3},25,'MarkerFaceColor','none','MarkerEdgeColor',colour_65Q,'MarkerEdgeAlpha',0.5,'LineWidth',2);
scatter(4-0.125+0.25*rand(numel(ant_vs_ret_all{4}),1),ant_vs_ret_all{4},25,'MarkerFaceColor','none','MarkerEdgeColor',colour_81Q,'MarkerEdgeAlpha',0.5,'LineWidth',2);
plot(1,mean(ant_vs_ret_all{1}),'k_','MarkerSize',10);
plot(2,mean(ant_vs_ret_all{2}),'k_','MarkerSize',10);
plot(3,mean(ant_vs_ret_all{3}),'k_','MarkerSize',10);
plot(4,mean(ant_vs_ret_all{4}),'k_','MarkerSize',10);
errorbar(1,mean(ant_vs_ret_all{1}),SEM_ant_ret{1},'Color',colour_30Q,'LineWidth',2,'CapSize',0);
errorbar(2,mean(ant_vs_ret_all{2}),SEM_ant_ret{2},'Color',colour_45Q,'LineWidth',2,'CapSize',0);
errorbar(3,mean(ant_vs_ret_all{3}),SEM_ant_ret{3},'Color',colour_65Q,'LineWidth',2,'CapSize',0);
errorbar(4,mean(ant_vs_ret_all{4}),SEM_ant_ret{4},'Color',colour_81Q,'LineWidth',2,'CapSize',0);
xlim([0 5]);
xticks([1 2 3 4]);
% xticklabels({'30Q','45Q', '65Q','81Q'});
    xticklabels({'30Q','30Q+IFNγ', '81Q','81Q+IFNγ'});
...
fileprefix='20240118_isoHD_KB_bdnf_IFg_flux_15um';
