...
% 
% This version calculates it a bit differently, only counting runs that go from
% one side of the box to the other as a particular flux.
...
for it=1:5
    ...
    ax_seg=(axon_length{k}*it)/5-(box_width+1);
...
%                 count_ent_then_stop=count_ent_then_stop+1; %counting this as a separate case
                count_ant_exits(it)=0; %counting the ones that entered from the soma side as anterograde flux
            end
...
%             traj_stays_in_box=all(position(idx_enters:end)> mid_length-box_width & position(idx_enters:end)<mid_length+box_width);
            ...
            elseif any(traj_exits_box_ant)
                count_ant_exits(it)=0;
            elseif any(traj_ent_then_stop)
%                 count_ent_then_stop=count_ent_then_stop+1; %counting this as a separate case
                count_ret_exits(it)=0; %counting those that enter from the distal side as retrograde flux
            end
...
%     plot(position, t_position,'-','Color',cmap(kd,:),'linewidth',2)
%     xlabel('Position (\mum)'), ylabel('Time (s)'), hold on
%     xlim([0,axon_length{k}]);
...
save([save_dir, 'KB_IFg_total_enters_1um_newdef'],'tot_enters_per_cell_all_conds');
save([save_dir, 'KB_IFg_frac_ant_ent_1um_newdef'],'frac_ant_enter_per_cell_all_conds');
save([save_dir, 'KB_IFg_frac_ret_ent_1um_newdef'],'frac_ret_enter_per_cell_all_conds');
save([save_dir, 'KB_IFg_start_in_1um_newdef'],'frac_starts_in_per_cell_all_conds');
save([save_dir, 'KB_IFg_frac_ant_ex_1um_newdef'],'frac_ant_exits_per_cell_all_conds');
save([save_dir, 'KB_IFg_frac_ret_ex_1um_newdef'],'frac_ret_exits_per_cell_all_conds');
save([save_dir, 'KB_IFg_frac_stay_in_1um_newdef'],'frac_stays_in_per_cell_all_conds');
...
scatter(1-0.125+0.25*rand(numel(tot_enters_per_cell_all_conds{1}),1),tot_enters_per_cell_all_conds{1},25,'MarkerFaceColor','none','MarkerEdgeColor',colour_30Q,'MarkerEdgeAlpha',0.5,'LineWidth',2);
scatter(2-0.125+0.25*rand(numel(tot_enters_per_cell_all_conds{2}),1),tot_enters_per_cell_all_conds{2},25,'MarkerFaceColor','none','MarkerEdgeColor',colour_45Q,'MarkerEdgeAlpha',0.5,'LineWidth',2);
scatter(3-0.125+0.25*rand(numel(tot_enters_per_cell_all_conds{3}),1),tot_enters_per_cell_all_conds{3},25,'MarkerFaceColor','none','MarkerEdgeColor',colour_65Q,'MarkerEdgeAlpha',0.5,'LineWidth',2);
scatter(4-0.125+0.25*rand(numel(tot_enters_per_cell_all_conds{4}),1),tot_enters_per_cell_all_conds{4},25,'MarkerFaceColor','none','MarkerEdgeColor',colour_81Q,'MarkerEdgeAlpha',0.5,'LineWidth',2);
plot(1,mean(tot_enters_per_cell_all_conds{1}),'k_','MarkerSize',10);
plot(2,mean(tot_enters_per_cell_all_conds{2}),'k_','MarkerSize',10);
plot(3,mean(tot_enters_per_cell_all_conds{3}),'k_','MarkerSize',10);
plot(4,mean(tot_enters_per_cell_all_conds{4}),'k_','MarkerSize',10);
errorbar(1,mean(tot_enters_per_cell_all_conds{1}),SEM_tot_ent{1},'Color',colour_30Q,'LineWidth',2,'CapSize',0);
errorbar(2,mean(tot_enters_per_cell_all_conds{2}),SEM_tot_ent{2},'Color',colour_45Q,'LineWidth',2,'CapSize',0);
errorbar(3,mean(tot_enters_per_cell_all_conds{3}),SEM_tot_ent{3},'Color',colour_65Q,'LineWidth',2,'CapSize',0);
errorbar(4,mean(tot_enters_per_cell_all_conds{4}),SEM_tot_ent{4},'Color',colour_81Q,'LineWidth',2,'CapSize',0);
xlim([0 5]);
xticks([1 2 3 4]);
xticklabels({'30Q','45Q', '65Q','81Q'});
% xticklabels({'30Q','30Q+IFNγ', '81Q','81Q+IFNγ'});
...
fileprefix='20240110_isoHD_KB_lyso_IFg_flux_1um_newdef';
