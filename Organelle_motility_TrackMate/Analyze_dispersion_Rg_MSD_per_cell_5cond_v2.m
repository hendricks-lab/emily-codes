...
tic
for k_choose = 5 %Don't forget to change min_lr based on your cargo! 0.5 for BDNF, 0.6 for mito, 0.8 for Lyso, also maybe change K to 10 for BDNF only
...
elseif k_choose == 2  % 30Q
    %Lyso
%     dat_dir = '/Volumes/Emily_htt_2/Neuron/isoHD30Q/30Q_lyso_proj_along_kymo_test/';
%     save_dir='/Volumes/Emily_htt_2/Neuron/isoHD30Q/30Q_lyso_proj_along_kymo_test/Results/';
%     save_dir_Rg='/Volumes/Emily_htt_2/Neuron/isoHD30Q/30Q_lyso_proj_along_kymo_test/Results/';
%     save_dir_MSD='/Volumes/Emily_htt_2/Neuron/isoHD30Q/30Q_lyso_proj_along_kymo_test/Results/';
%     save_dir_dirbias=['/Volumes/Emily_htt_2/Neuron/isoHD30Q/30Q_lyso_proj_along_kymo_test/Results/'];
...
min_lr=0.6; %0.5 for BDNF, 0.6 for mito, 0.8 for lyso
numel(fls);
...
%         like they have negative slope I guess
        ...
        dat_pos_tim{kf}=[];
            ...
            dat_pos_tim{j}=[]; %Emily added 20230817 when optimizing for the position projection along kymograph coordinates code
        end
    end %the end of the function analyzing trajectories
    alpha_2d{k}=[mean(alphas)]; %Emily added 20230720
    pos=[pos,dat_pos_tim];
    position{k}=pos;
%     axon_length_per_cell{k}=ab(k).axon_length;
    axon_length_per_cell{k}=ab(1).axon_length; 

    save([save_dir, num2str(k_choose) 'Position_per_cell'],'position');
    
    % Clear 1D MSD
    K=1:40; %Emily-Maybe we should use the 2D MSD instead of this 1D one, since I'm getting the NaNs in this analysis
    Ndat=numel(xk); %Emily-I think the problem is that xk is getting bigger with each run through the loop and therefore Ndat is increasing even though it shouldn't
